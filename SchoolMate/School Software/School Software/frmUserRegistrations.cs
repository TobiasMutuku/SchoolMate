    ...
    public partial class frmUserRegistrations : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (txtDesignation.Text == "Super Admin")
            {
                con = new SqlConnection(cs.ReadfromXML());
                con.Open();
                string ct = "SELECT Designations.Designation FROM Designations INNER JOIN Users ON Designations.DesignationID = Users.Designation_ID WHERE (Designations.Designation = 'Super Admin') ";
        ...
        private void delete_records()
        {
            try
            {
                ...
                con = new SqlConnection(cs.ReadfromXML());
                con.Open();
                string ctm1 = "select UserID from UserGrants where UserID='" + txtID.Text + "'";
                cmd = new SqlCommand(ctm1);
                cmd.Connection = con;
                rdr = cmd.ExecuteReader();
                if (rdr.Read())
                {
                    MessageBox.Show("Action can't be Completed Because this User using on UserGrant Form..!!", "Record In Use", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Reset();
                    txtUserID.Focus();
                    if ((rdr != null))
                    {
        ...
        private void btnUpdate_record_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((txtDesignation.Text) != (textBox1.Text))
                {
                if (txtDesignation.Text == "Super Admin")
                {
                    con = new SqlConnection(cs.ReadfromXML());
